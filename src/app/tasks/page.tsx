...
export default function TasksPage() {
  ...
  const filtered = tasks.filter(t => {
    const matchesSearch = !searchQuery || 
      ...
      (t.lead?.leadName.toLowerCase().includes(searchQuery.toLowerCase())) ||
    ...
    return matchesSearch && matchesStatus && matchesDept && matchesUser && matchesDate && matchesCategory;
  });
                ...
                {filtered.map(task => {
                  ...
                  return (
                    <tr key={task.id}>
                      ...
                      <td>
                         <span style={{ fontWeight: 600 }}>{getLeadName(task)}</span>
          ...
          {tasks.find(t => t.id === selectedTask) && (
            <div style={{ padding: '0.25rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.5rem', fontSize: '0.85rem' }}>
                <p style={{ color: 'var(--text-secondary)' }}>Lead: <strong style={{ color: 'var(--text-primary)' }}>{getLeadName(tasks.find(t => t.id === selectedTask)!)}</strong></p>
